...
export const genArgType = (description: string, control?: string, defaultValue?: string) => ({
  description,
...
export const basicArgTypes = {
  ...
  pagination: {
    ...genArgType(
      'Go to Molecules/Pagination. Allowed props: current, pageSize, total, simple, onChange, onShowSizeChange.' +
  ...
  toolbar: {
    ...genArgType(
      'Go to Molecules/Toolbar. External fields:<br/>' +
...
export const basicDataSource = Array.from({ length: 180 }).map((_, index): BasicRowType & { key: number } => ({
  name: `Value ${index + 1}`,
...
export type BasicTreeRowType = BasicRowType & { age: number, children?: BasicTreeRowType[] }
