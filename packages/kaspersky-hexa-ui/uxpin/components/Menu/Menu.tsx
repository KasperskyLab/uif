...
import { Hamburger, Menu as HexaMenu, ServicesNav } from '@src/menu'
...
import { noop } from '../../preview'

import { BookmarkOutline, Help, Moon, SettingsGear } from '@kaspersky/hexa-ui-icons/16'
...
const DefaultLogoMark = (): JSX.Element => (
  <svg width="64" height="64" viewBox="0 0 128 128" fill="none" xmlns="http://www.w3.org/2000/svg">
    <g clipPath="url(#uxpinMenuLogoClip0)">
      <g clipPath="url(#uxpinMenuLogoClip1)">
    ...
    <defs>
      <clipPath id="uxpinMenuLogoClip0">
        <rect width="128" height="128" fill="white"/>
      </clipPath>
      <clipPath id="uxpinMenuLogoClip1">
        <rect width="128" height="128" fill="white"/>
      </clipPath>
    </defs>
  </svg>
...
const cloneNavItems = (items: NavItemData[] = []): NavItemData[] => (
  items.map((item) => ({
    ...item,
...
const buildPreviewUserItems = (): NavItemData[] => {
  const items = cloneNavItems(storyNavUserItems)
  const settingsItem = items.find((item) => item.state === 'settings' || item.key === 'Settings')
  const userItem = items.find((item) => item.state === 'user')

  if (settingsItem) {
    settingsItem.icon = <SettingsGear />
  }
...
const DEFAULT_PREVIEW_USER_ITEMS = buildPreviewUserItems()
  ...
  return (
    <PreviewRoot>
      <HexaMenu
        ...
        navUserItems={footer ? cloneNavItems(DEFAULT_PREVIEW_USER_ITEMS) : undefined}
        favIcon={<BookmarkOutline />}
