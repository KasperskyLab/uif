...
const meta: Meta<ITableProps<TableException>> = {
  title: 'Hexa UI Components/Table/Reductions in cells',
  component: Table,
  args: {
    columns,
...
const generalInfo = `
### Ограничение длины контента в ячейках 
...
Компонент TextReducer :
- автоматически обрезает текст при превышении ширины колонки (через CSS свойства)
...
export const RowEllipsis: Story = {
  render: (args) => {
    return (
      <Wrapper>
...
export const ExpandableText: Story = {
  render: (args) => {
    const [dataSourсe, setDataSourсe] = useState(basicDataSource.slice(0, 20).map(({ name, ...rest }) => ({
      ...rest,
      name: MOCK_LONG_TEXT,
      description: MOCK_LONG_TEXT
    })))
    return (
      <BasicTableStory
        {...args}
        dataSource={dataSourсe}
        onDragEnd={(rows: any) => { setDataSourсe(rows) }}
      />
    )
  },
  args: {
