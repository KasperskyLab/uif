...
const selectAllPageSize = 20
const selectAllSmallData = generatedData.slice(0, 5)
...
const renderWithRowSelection = (
  ...
  columns: tableColumns,
  pagination: { pageSize: selectAllPageSize, ...pagination },
  testId: TABLE_TEST_ID,
  rowSelection: { processSelection: jest.fn(), builtInRowSelection: true }
})

describe('Select all with disabled rows on current page', () => {
  type DisabledVariant = 'few' | 'page' | 'all'

  const disableVariants: Record<DisabledVariant, (rows: MockRow[]) => MockRow[]> = {
    few: rows => rows.map((row, index) => index < 3 ? { ...row, _selectionDisabled: true } : row),
    page: rows => rows.map((row, index) => index < selectAllPageSize ? { ...row, _selectionDisabled: true } : row),
    all: rows => rows.map(row => ({ ...row, _selectionDisabled: true }))
  }
  ...
  const sizeCases = [
    // client/server отдают данные целиком (data), pseudo-server — уже нарезанную страницу (sliced) + total.
    { name: 'rows fewer than pageSize', data: selectAllSmallData, sliced: selectAllSmallData, total: selectAllSmallData.length },
    { name: 'rows more than pageSize', data: selectAllLargeData, sliced: selectAllLargeSlicedData, total: selectAllLargeData.length }
  ] as const

  // Ожидаемое состояние чекбокса SelectAll: [mode][sizeIndex][disabledVariant]
  // размер: 0 = fewer, 1 = more
  const expectedSelectAll: Record<string, Record<number, Record<DisabledVariant, boolean>>> = {
    client: {
      0: { few: false, page: true, all: true },
      1: { few: false, page: false, all: true }
    },
    server: {
      0: { few: false, page: true, all: true },
      1: { few: false, page: false, all: false }
    },
    'pseudo-server': {
      0: { few: false, page: true, all: true },
      1: { few: false, page: false, all: false }
  ...
  const renderMode = (mode: string, size: typeof sizeCases[number], disabledRows: MockRow[]) => {
    if (mode === 'server') return renderWithRowSelection('server', disabledRows)
    if (mode === 'pseudo-server') {
      // pseudo-server: наружу отдают только видимую страницу, total задан явно.
      return renderWithRowSelection('client', disabledRows.filter((_, i) => i < selectAllPageSize), { total: size.total })
    }
    return renderWithRowSelection('client', disabledRows)
  }

  const disabledCases = paginationModes.flatMap(mode =>
    sizeCases.flatMap((size, sizeIndex) =>
      (['few', 'page', 'all'] as DisabledVariant[]).map(variant => {
        const rows = disableVariants[variant](size.data)
        const description = `${mode.name} pagination, ${variant} rows disabled, ${size.name}`
        return {
          description,
          expectSelectAllDisabled: expectedSelectAll[mode.name][sizeIndex][variant],
          expectCurrentPageDisabled: variant !== 'few',
          render: () => renderMode(mode.name, size, rows)
        }
      })))

  describe.each(disabledCases)('$description', (currentCase) => {
    it('should render the Select all checkbox with the expected disabled state', async () => {
      const { table } = await currentCase.render()

      const checkbox = table.selection.getHeaderCheckbox()
      expect(checkbox).toBeInTheDocument()
      if (currentCase.expectSelectAllDisabled) {
    ...
    it('should expose Select all on current page consistent with disabled rows on the page', async () => {
      const { table } = await currentCase.render()
      await table.rows.waitForData()

      if (currentCase.expectSelectAllDisabled) {
        fireEvent.click(table.selection.getHeaderCheckbox()!)
        expect(screen.queryByTestId('select-current-page')).not.toBeInTheDocument()
        return
      }

      await table.selection.openHeaderMenu()
      const currentPageOption = await screen.findByTestId('select-current-page')
      if (currentCase.expectCurrentPageDisabled) {
        expect(currentPageOption).toHaveAttribute('aria-disabled', 'true')
      } else {
        expect(currentPageOption).toHaveAttribute('aria-disabled', 'false')
      }
    })
  })
})