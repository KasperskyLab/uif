...
const defaultProps = {
  options: [
    { label: 'first option', value: '1', 'kl-value': '1', 'kl-id': '1' },
...
const getFirstOption = (container: HTMLElement) => container.querySelector('.ant-select-item') as HTMLDivElement
...
function getSelectedLabel (container: HTMLElement) {
  return container.querySelector('.ant-select-selection-item')?.innerHTML
}
...
describe('Select', () => {
  ...
  test('should not auto clear search value with autoClearSearchValue false', async () => {
    ...
    getFirstOption(container).click()
  ...
  it('should change value if Select is used as uncontrolled component', async () => {
    ...
    expect(getSelectedLabel(container)).toBe(defaultProps.options[0].label)
  })
  ...
  test('should receive test attributes', async () => {
    ...
    const backwardCompatibility = container.querySelector('[kl-value="1"]')
    const backwardCompatibility2 = container.querySelector('[kl-id="1"]')

    expect(select).toBeInTheDocument()
    expect(dropdown).toBeInTheDocument()
    expect(firstItem).toBeInTheDocument()
    expect(backwardCompatibility).toBeInTheDocument()
    expect(backwardCompatibility2).toBeInTheDocument()
  })

  test('should receive test attributes if options are rendered as children', async () => {
    const testId = 'test-select'

    const { container } = render(
      <ConfigProvider>
        <Select {...defaultProps} open options={undefined} testId={testId}>
          {defaultProps.options.map(option => (
            <Select.Option key={option.value} {...option} data-value={option.value}>
              {option.label}
            </Select.Option>
          ))}
        </Select>
