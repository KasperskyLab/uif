...
describe('Input - Textbox - Password ', () => {
  ...
  test('clears the password input value before first change', async () => {
    ...
    const { getByTestId, rerender } = render(
      <Textbox.Password klId={klId} clearBeforeFirstChange onChange={onChange} value={value} />
    )
    const passwordInput = getByTestId(klId)
    expect(passwordInput).toHaveValue('oldPassword')
    fireEvent.focus(passwordInput)
    await waitFor(() => { expect(passwordInput).not.toHaveValue() })
    fireEvent.change(passwordInput, { target: { value: 'newPassWord' } })
    rerender(
      <Textbox.Password klId={klId} clearBeforeFirstChange onChange={onChange} value={value} />
    )
    await waitFor(() => { expect(passwordInput).toHaveValue('newPassWord') })
  })
})
...
describe('Input - Textbox - Url ', () => {
  ...
  test('displays an error when error prop is passed', () => {
    const klId = 'url-input-id'
    const { getByTestId } = render(<Textbox.Url klId={klId} validationStatus="error" />)
    const pathInput = getByTestId('kl-v6-path-input')
    const portInput = getByTestId('kl-v6-port-input')
