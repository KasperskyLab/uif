...
import { popoverStorySettings } from './Popover.stories'
import { StickyTabsPopoverWrapper } from './PopoverStickyTabsWrapper'
...
const StickyTabsTabContent: React.FC = () => (
  <Space direction="vertical" gap={12} width="100%">
    {Array.from({ length: 8 }).map((_, index) => (
      <Textbox key={index} placeholder={`Field ${index + 1}`} />
    ))}
    <FillText />
    <Link text="Online help" decoration="icon" size="medium" />
  </Space>
...
const StickyTabsPopoverContent: React.FC = () => (
  <StickyTabsPopoverWrapper>
    <Tabs defaultActiveKey="1" noMargin padding={false}>
      <Tabs.TabPane tab="General" key="1">
        <StickyTabsTabContent />
      </Tabs.TabPane>
...
const meta: Meta<typeof Popover> = {
  title: 'Hexa UI Components/Popover/Stories',
  component: Popover,
  tags: ['!autodocs'],
...
export const WithStickyTabs: Story = {
  render: (args: PopoverProps) => (
    <div style={{ paddingLeft: '300px', paddingTop: '250px' }}>
      <Space gap={10}>
