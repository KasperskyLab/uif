...
Документ описывает, как в `kaspersky-hexa-ui` формируется набор пропов для вкладки `Playground`, как задаётся порядок controls и какими способами скрывать ненужные пропы.
