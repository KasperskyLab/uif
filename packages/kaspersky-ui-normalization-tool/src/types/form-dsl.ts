...
/** Создаёт пустую форму (используется при закрытии, инициализации, parseFormJs fallback) */
 ...
 * Выполняет модуль через import(blobUrl); обработчики кнопок могут быть () => import("./path").
 * Примечание: при необходимости безопасного парсинга без выполнения кода (например, только JSON)
 * стоит ввести отдельную функцию/режим.
 */
...
/** Сериализует форму в исходный код JS-модуля. Обработчик кнопки — динамический импорт: () => import("./path"). */
export function formToJs(form: FormData): string {
...
/** Форматирует значение для JS-исходника: примитивы в одну строку, объекты/массивы — с переносами и отступами. */
