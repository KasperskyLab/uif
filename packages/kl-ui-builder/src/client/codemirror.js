...
(function (mod) {
  ...
  function updateGutters (cm) {
    const gutters = cm.display.gutters; const specs = cm.options.gutters
  ...
  NativeScrollbars.prototype = copyObj({
    ...
    enableZeroWidthBar: function (bar, delay) {
      ...
      function maybeDisable () {
        ...
        const elt = document.elementFromPoint(box.left + 1, box.bottom - 1)
  ...
  function updateHeightsInViewport (cm) {
    ...
    for (let i = 0; i < display.view.length; i++) {
      const cur = display.view[i]; var height
  ...
  function updateWidgetHeight (line) {
    if (line.widgets) {
      for (let i = 0; i < line.widgets.length; ++i) { line.widgets[i].height = line.widgets[i].node.parentNode.offsetHeight }
    }
  }
  ...
  function updateLineGutter (cm, lineView, lineN, dims) {
    ...
    if (cm.options.lineNumbers || markers) {
      var wrap = ensureLineWrapped(lineView)
      const gutterWrap = lineView.gutter = elt('div', null, 'CodeMirror-gutter-wrapper', 'left: ' +
      ...
      if (markers) {
        for (let k = 0; k < cm.options.gutters.length; ++k) {
          const id = cm.options.gutters[k]; const found = markers.hasOwnProperty(id) && markers[id]
          if (found) {
  ...
  function insertLineWidgetsFor (cm, line, lineView, dims, allowAbove) {
    ...
    for (let i = 0, ws = line.widgets; i < ws.length; ++i) {
      const widget = ws[i]; const node = elt('div', [widget.node], 'CodeMirror-linewidget')
  ...
  const cmp = CodeMirror.cmpPos = function (a, b) { return a.line - b.line || a.ch - b.ch }
  ...
  function applyTextInput (cm, inserted, deleted, sel, origin) {
    ...
    const textLines = doc.splitLines(inserted); let multiPaste = null
  ...
  function hiddenTextarea () {
    const te = elt('textarea', null, null, 'position: absolute; padding: 0; width: 1px; height: 1em; outline: none')
  ...
  TextareaInput.prototype = copyObj({
    ...
    reset: function (typing) {
      ...
      let minimal; let selected; const cm = this.cm; const doc = cm.doc
      if (cm.somethingSelected()) {
        this.prevInput = ''
  ...
  ContentEditableInput.prototype = copyObj({
    init: function (display) {
      ...
      on(div, 'compositionstart', function (e) {
        ...
        const line = cm.getLine(prim.head.line)
        const found = line.indexOf(data, Math.max(0, prim.head.ch - data.length))
    ...
    showPrimarySelection: function () {
      const sel = window.getSelection(); const prim = this.cm.doc.sel.primary()
      const curAnchor = domToPos(this.cm, sel.anchorNode, sel.anchorOffset)
      const curFocus = domToPos(this.cm, sel.focusNode, sel.focusOffset)
      ...
      const view = this.cm.display.view
    ...
    pollSelection: function () {
      if (!this.composing && !this.gracePeriod && this.selectionChanged()) {
        const sel = window.getSelection(); const cm = this.cm
        this.rememberSelection()
  ...
  function posToDOM (cm, pos) {
    ...
    const order = getOrder(line); let side = 'left'
    if (order) {
  ...
  function domTextBetween (cm, from, to, fromLine, toLine) {
    let text = ''; let closing = false; const lineSep = cm.doc.lineSeparator()
    ...
    if (line.markedSpans) {
      for (let i = 0; i < line.markedSpans.length; ++i) {
        ...
        if ((sp.from == null || (m.inclusiveLeft ? sp.from <= pos.ch : sp.from < pos.ch)) &&
          ...
          if (oldPos) {
            let near = m.find(dir < 0 ? 1 : -1); var diff
  ...
  function drawSelectionRange (cm, range, output) {
    ...
    function drawForLine (line, fromArg, toArg) {
      ...
      iterateBidiSections(getOrder(lineObj), fromArg || 0, toArg == null ? lineLen : toArg, function (from, to, dir) {
        let leftPos = coords(from, 'left'); let rightPos; let left; let right
        if (from == to) {
          rightPos = leftPos
  ...
  function highlightWorker (cm) {
    ...
    doc.iter(doc.frontier, Math.min(doc.first + doc.size, cm.display.viewTo + 500), function (line) {
      if (doc.frontier >= cm.display.viewFrom) { // Visible
        const oldStyles = line.styles; const tooLong = line.text.length > cm.options.maxHighlightLength
        const highlighted = highlightLine(cm, line, tooLong ? copyState(doc.mode, state) : state, true)
  ...
  function getStateBefore (cm, n, precise) {
    ...
    let pos = findStartLine(cm, n, precise); let state = pos > doc.first && getLine(doc, pos - 1).stateAfter
  ...
  function cursorCoords (cm, pos, context, lineObj, preparedMeasure, varHeight) {
    ...
    function getBidi (ch, partPos) {
      let part = order[partPos]; let right = part.level % 2
    ...
    var order = getOrder(lineObj); const ch = pos.ch
  ...
  function estimateCoords (cm, pos) {
    let left = 0; var pos = clipPos(cm.doc, pos)
  ...
  function coordsCharInner (cm, lineObj, lineNo, x, y) {
    ...
    let wrongLine = false; const adjust = 2 * cm.display.wrapper.clientWidth
    ...
    const bidi = getOrder(lineObj); let dist = lineObj.text.length
    let from = lineLeft(lineObj); let to = lineRight(lineObj)
    let fromX = getX(from); let fromOutside = wrongLine; let toX = getX(to); let toOutside = wrongLine
    ...
    for (;;) {
      if (bidi ? to == from || to == moveVisually(lineObj, from, 1) : to - from <= 1) {
        ...
        if (toOutside && !bidi && !/\s/.test(lineObj.text.charAt(ch)) && xDiff > 0 &&
          ...
          const charSize = measureCharPrepared(cm, preparedMeasure, ch, 'right')
        ...
        const pos = PosWithInfo(lineNo, ch, outside, xDiff < -1 ? -1 : xDiff > 1 ? 1 : 0)
        return pos
      }
      const step = Math.ceil(dist / 2); let middle = from + step
      if (bidi) {
        middle = from
  ...
  function endOperation (cm) {
    const op = cm.curOp; const group = op.ownsGroup
    if (!group) return
  ...
  function dragDropChanged (cm, value, old) {
    const wasOn = old && old != CodeMirror.Init
    if (!value != !wasOn) {
  ...
  function leftButtonDown (cm, e, start) {
    ...
    const sel = cm.doc.sel; const modifier = mac ? e.metaKey : e.ctrlKey; let contained
  ...
  function leftButtonStartDrag (cm, e, start, modifier) {
    const display = cm.display; const startTime = +new Date()
  ...
  function leftButtonSelect (cm, e, start, type, addNew) {
    ...
    function extendTo (pos) {
      ...
      if (type == 'rect') {
        var ranges = []; const tabSize = cm.options.tabSize
        ...
        let anchor = oldRange.anchor; let head = pos
        if (type != 'single') {
  ...
  function onDrop (e) {
    ...
    if (files && files.length && window.FileReader && window.File) {
      const n = files.length; var text = Array(n); let read = 0
  ...
  const wheelEventDelta = function (e) {
  ...
  function makeChange (doc, change, ignoreReadOnly) {
    ...
    if (split) {
      for (let i = split.length - 1; i >= 0; --i) { makeChangeInner(doc, { from: split[i].from, to: split[i].to, text: i ? [''] : change.text }) }
    } else {
  ...
  function scrollPosIntoView (cm, pos, end, margin) {
    ...
    for (let limit = 0; limit < 5; limit++) {
      let changed = false; var coords = cursorCoords(cm, pos)
  ...
  function calculateScrollPos (cm, x1, y1, x2, y2) {
    ...
    const atTop = y1 < snapMargin; const atBottom = y2 > docBottom - snapMargin
    if (y1 < screentop) {
  ...
  function ensureCursorVisible (cm) {
    ...
    const cur = cm.getCursor(); let from = cur; let to = cur
    if (!cm.options.lineWrapping) {
  ...
  function findPosH (doc, pos, dir, unit, visually) {
    let line = pos.line; let ch = pos.ch; const origDir = dir
    ...
    const result = skipAtomic(doc, Pos(line, ch), pos, origDir, true)
  ...
  CodeMirror.prototype = {
    ...
    clearGutter: methodOp(function (gutterID) {
      const cm = this; const doc = cm.doc; let i = doc.first
      doc.iter(function (line) {
    ...
    moveV: methodOp(function (dir, unit) {
      const cm = this; const doc = this.doc; const goals = []
    ...
    setSize: methodOp(function (width, height) {
      ...
      cm.doc.iter(lineNo, cm.display.viewTo, function (line) {
        if (line.widgets) {
          for (let i = 0; i < line.widgets.length; i++) { if (line.widgets[i].noHScroll) { regLineChange(cm, lineNo, 'widget'); break } }
        }
        ++lineNo
      })
      cm.curOp.forceUpdate = true
  ...
  option('keyMap', 'default', function (cm, val, old) {
    ...
    const prev = old != CodeMirror.Init && getKeyMap(old)
  ...
  const modes = CodeMirror.modes = {}; const mimeModes = CodeMirror.mimeModes = {}
  ...
  function normalizeKeyName (name) {
    const parts = name.split(/-(?!$)/); var name = parts[parts.length - 1]
  ...
  var keyName = CodeMirror.keyName = function (event, noShift) {
    ...
    const base = keyNames[event.keyCode]; let name = base
  ...
  CodeMirror.fromTextArea = function (textarea, options) {
    ...
    if (textarea.form) {
      ...
      if (!options.leaveSubmitMethodAlone) {
        const form = textarea.form; var realSubmit = form.submit
        try {
  ...
  TextMarker.prototype.clear = function () {
    ...
    if (cm && this.collapsed && !cm.options.lineWrapping) {
      for (var i = 0; i < this.lines.length; ++i) {
        const visual = visualLine(this.lines[i]); const len = lineLength(visual)
  ...
  function detachSharedMarkers (markers) {
    for (let i = 0; i < markers.length; i++) {
      const marker = markers[i]; var linked = [marker.primary.doc]
  ...
  const LineWidget = CodeMirror.LineWidget = function (doc, node, options) {
    if (options) {
  ...
  function takeToken (cm, pos, precise, asArray) {
    ...
    var doc = cm.doc; const mode = doc.mode; let style
    pos = clipPos(doc, pos)
    const line = getLine(doc, pos.line); var state = getStateBefore(cm, pos.line, precise)
    var stream = new StringStream(line.text, cm.options.tabSize); let tokens
    if (asArray) tokens = []
  ...
  function runMode (cm, text, mode, state, f, lineClasses, forceToEnd) {
    ...
    const stream = new StringStream(text, cm.options.tabSize); let style
  ...
  function getLineStyles (cm, line, updateFrontier) {
    if (!line.styles || line.styles[0] != cm.state.modeGen) {
      ...
      const result = highlightLine(cm, line, line.text.length > cm.options.maxHighlightLength ? copyState(cm.doc.mode, state) : state)
  ...
  function buildLineContent (cm, lineView) {
    ...
    for (let i = 0; i <= (lineView.rest ? lineView.rest.length : 0); i++) {
      const line = i ? lineView.rest[i - 1] : lineView.line; var order
      builder.pos = 0
  ...
  function buildToken (builder, text, style, startStyle, endStyle, title, css) {
    if (!text) return
    const displayText = builder.splitSpaces ? text.replace(/ {3,}/g, splitSpaces) : text
    ...
    if (!special.test(text)) {
      ...
      var content = document.createDocumentFragment(); let pos = 0
      while (true) {
  ...
  function insertLineContent (line, builder, styles) {
    ...
    const len = allText.length; let pos = 0; var i = 1; let text = ''; let style; let css
    let nextChange = 0; let spanStyle; let spanEndStyle; let spanStartStyle; let title; let collapsed
    for (;;) {
      if (nextChange == pos) { // Update current marker set
        spanStyle = spanEndStyle = spanStartStyle = title = css = ''
        collapsed = null; nextChange = Infinity
        const foundBookmarks = []; var endStyles
  ...
  Doc.prototype = createObj(BranchChunk.prototype, {
    ...
    linkedDoc: function (options) {
      ...
      const copy = new Doc(getLines(this, from, to), options.mode || this.modeOption, from, this.lineSep)
  ...
  function lineAtHeight (chunk, h) {
    ...
    outer: do {
      for (var i = 0; i < chunk.children.length; ++i) {
        const child = chunk.children[i]; const ch = child.height
  ...
  const e_stopPropagation = CodeMirror.e_stopPropagation = function (e) {
  ...
  var off = CodeMirror.off = function (emitter, type, f) {
    if (emitter.removeEventListener) { emitter.removeEventListener(type, f, false) } else if (emitter.detachEvent) { emitter.detachEvent('on' + type, f) } else {
      const handlers = getHandlers(emitter, type, false)
      for (let i = 0; i < handlers.length; ++i) { if (handlers[i] == f) { handlers.splice(i, 1); break } }
    }
  }

  var signal = CodeMirror.signal = function (emitter, type /*, values... */) {
    const handlers = getHandlers(emitter, type, true)
  ...
  function signalLater (emitter, type /*, values... */) {
    const arr = getHandlers(emitter, type, false)
    ...
    for (let i = 0; i < arr.length; ++i) { list.push(bnd(arr[i])) }
  }
  ...
  const isWordCharBasic = CodeMirror.isWordChar = function (ch) {
  ...
  function lineEnd (cm, lineN) {
    let merged; let line = getLine(cm.doc, lineN)
    while (merged = collapsedSpanAtEnd(line)) {
      line = merged.find(1, true).line
      lineN = null
    }
  ...
  function moveVisually (line, start, dir, byUnit) {
    ...
    let pos = getBidiPartAt(bidi, start); let part = bidi[pos]
    ...
    return function (str) {
      ...
      for (var i = 0; i < len;) {
        if (countsAsLeft.test(types[i])) {
          ...
          let pos = i; const at = order.length
