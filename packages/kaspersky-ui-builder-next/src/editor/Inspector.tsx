...
} from "../domain/component-registry.ts"
import type {
  Condition,
  ConditionExpression,
  ConditionGroup,
  ...
  return (
    <>
      ...
      <div className="inspector-scroll">
        ...
        <InspectorGroup title="Внешний вид контрола">
          {(
            [
              "nestingLevel",
              "marginTop",
          ...
          <FieldLabel title="Ширина контрола">
            <input
              min="0"
              type="number"
        ...
        return (
          <div className="structured-card" key={dependency.id}>
            ...
            <FieldLabel title="Свойство">
              ...
              >
                {["visible", "disabled", "readOnly", "loading"].map((item) => (
                  <option key={item}>{item}</option>
                ))}
              </select>
            </FieldLabel>
    ...
    <>
      ...
      <div className="inspector-scroll">
        <InspectorGroup title="Основные настройки" open>
          ...
          <FieldLabel title="Название формы">
            <input
              value={draft.name}
              onChange={(event) =>
          ...
          <FieldLabel title="Заголовок формы">
            <input
          ...
          <FieldLabel title="Тип объекта" optional>
            <select
              value={settings.objectType ?? ""}
              onChange={(event) =>
                update({
                  objectType: (event.target.value ||
                    null) as FormSettings["objectType"],
                })
              }
            >
        ...
        <InspectorGroup title="Данные">
          <FieldLabel title="Источник данных">
            <select
          ...
          <FieldLabel title="Сохранение данных">
            <select
              value={dataSave.type}
              onChange={(event) =>
                update({
        ...
        <InspectorGroup title="Обработчики событий">
          ...
          <ToggleFunctionField
            ...
            title="После рендеринга формы запустить функцию"
            value={settings.afterRender}
          />
          <ToggleFunctionField
            onChange={(value) => update({ onClose: value })}
            title="При закрытии формы запустить функцию"
            value={settings.onClose}
          />
        </InspectorGroup>
        <InspectorGroup title="Внешний вид">
          <FieldLabel title="Тип отображения формы">
            <select
          ...
          <FieldLabel title="Родительская форма" optional>
            <select
