...
import { ChartTooltipWrapper, ChartTooltipWrapperProps } from './ChartTooltipWrapper/ChartTooltipWrapper'
...
function GenericLineChart<T> ({
  ...
  const { xTickFormatWithLocale, yTickFormatWithLocale } = useMemo(
    () => ({
      xTickFormatWithLocale: xTickFormat ? wrapCustomTickFormat(xTickFormat) : tickNumberOrDateFormat(locale),
      yTickFormatWithLocale: yTickFormat || tickNumberOrDateFormat(locale)
    }),
    [locale, xTickFormat, yTickFormat]
  )
  ...
  return (
    <>
      ...
      >
        ...
        <VictoryAxis
          dependentAxis
          tickLabelComponent={yTickHide ? <></> : <AxisLabel maxLength={maxYTextLength} />}
