...
import { defaultDimensions, Dimensions } from './types/Dimensions'
import { GaugeContext } from './types/GaugeContext'
import { defaultGaugeProps, GaugeProps, GaugeType, getGaugeMarginByType } from './types/GaugeProps'
import { defaultPointerRef, PointerRef } from './types/Pointer'

/*
Gauge creates a gauge chart using D3
...
export const Gauge: FC<GaugeProps> = (props) => {
  ...
  const mergedProps = useRef<GaugeProps>(props as GaugeProps)
  const prevProps = useRef<any>({})
  const resizeObserver = useRef<any>({})
  const selectedRef = useRef<any>(null)

  const gauge: GaugeContext = useMemo(() => ({
  ...
  const updateMergedProps = useCallback(() => {
