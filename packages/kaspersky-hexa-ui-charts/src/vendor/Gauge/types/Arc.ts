import { defaultGaugeProps, GaugeType } from './GaugeProps'
import { Tooltip } from './Tooltip'

export interface Arc {
  /** The corner radius of the arc. */
  cornerRadius?: number | ((d: unknown) => number),
...
export const getArcWidthByType = (type: string): number => {
  const gaugeTypesWidth: Record<string, number> = {
    [GaugeType.GRAFANA]: 0.25,
    [GaugeType.SEMICIRCLE]: 0.15,
    [GaugeType.RADIAL]: 0.2
  }
  if (!type) {
    type = defaultGaugeProps.type as string
  }
