...
import { GaugeInnerMarginInPercent, GaugeType } from '../types/GaugeProps'
...
export const initChart = (gauge: GaugeContext, isFirstRender: boolean): void => {
...
export const calculateAngles = (gauge: GaugeContext): void => {
  const { angles } = gauge.dimensions.current
...
export const renderChart = (gauge: GaugeContext, resize = false): void => {
  ...
  if (resize) {
    ...
    const ticksChanged =
      JSON.stringify(gauge.prevProps.current.labels?.tickLabels) !== JSON.stringify(labels.tickLabels)
...
export const updateDimensions = (gauge: GaugeContext): void => {
  ...
  const marginLeft: number = isMarginBox
    ? (marginInPercent as number)
    : (marginInPercent as GaugeInnerMarginInPercent).left
  const marginRight: number = isMarginBox
    ? (marginInPercent as number)
    : (marginInPercent as GaugeInnerMarginInPercent).right
  const marginTop: number = isMarginBox
    ? (marginInPercent as number)
    : (marginInPercent as GaugeInnerMarginInPercent).top
  const marginBottom: number = isMarginBox
    ? (marginInPercent as number)
...
export const calculateRadius = (gauge: GaugeContext): void => {
  const { dimensions } = gauge
  ...
  if (dimensions.current.width < 2 * dimensions.current.height) {
    ...
    dimensions.current.outerRadius =
      (dimensions.current.width - dimensions.current.margin.left - dimensions.current.margin.right) / 2
  } else {
    dimensions.current.outerRadius =
...
export const centerGraph = (gauge: GaugeContext): void => {
  const { dimensions } = gauge
...
export const clearChart = (gauge: GaugeContext): void => {
