...
(function (mod) {
  ...
  function maybeBackup (stream, pat, style) {
    const cur = stream.current(); const close = cur.search(pat)
    if (close > -1) {
  ...
  function addTags (from, to) {
    for (const tag in from) {
      ...
      for (let i = source.length - 1; i >= 0; i--) { dest.unshift(source[i]) }
    }
  }
  ...
  CodeMirror.defineMode('htmlmixed', function (config, parserConfig) {
    ...
    const tags = {}
    const configTags = parserConfig && parserConfig.tags; const configScript = parserConfig && parserConfig.scriptTypes
    addTags(defaultTags, tags)
    if (configTags) addTags(configTags, tags)
    if (configScript) {
      for (let i = configScript.length - 1; i >= 0; i--) { tags.script.unshift(['type', configScript[i].matches, configScript[i].mode]) }
    }

    function html (stream, state) {
      const style = htmlMode.token(stream, state.htmlState); const tag = /\btag\b/.test(style); let tagName
      if (tag && !/[<>\s\/]/.test(stream.current()) &&
        ...
        const endTagA = getTagRegexp(inTag[1], true); const endTag = getTagRegexp(inTag[1], false)
