...
| `form.model-binding.contract.exports` | Модуль экспортирует явный список допустимых путей, например **`export const MODEL_PATHS = ['a', 'a.b', 'items.0.title'] as const`** и при необходимости **`export type ModelPath = (typeof MODEL_PATHS)[number]`**. Список — **единственный** источник опций выпадающего списка в tool. |
| `form.model-binding.contract.validation` | При сохранении / смене **`dataBindPath`** в UI: если задан контракт и путь **не** входит в **`MODEL_PATHS`** — предупреждение или блок сохранения (уровень строгости — на реализации; в плане минимум — **предупреждение**). |
...
| `form.model-binding.tool.panel` | В **`PropertiesPanel`**: для **каждого** типа контрола — блок «Модель»: выпадающий список из **`MODEL_PATHS`** (после загрузки **`*.contract.ts`**); под списком **только чтение** текущего **`value`** для выбранного пути. |
...
| `form.model-binding.demo` | Обновить **`dsl/demo`**: **`model/demo.contract.ts`**, ссылка в **`demo.schema.ts`**, **`model/demo.data.ts`** (**`onFormInit`**) наполняет **`state`** согласованно с **`MODEL_PATHS`**, один контрол с биндингом. |
...
- Строгость валидации пути против **`MODEL_PATHS`**.  
