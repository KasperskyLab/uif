...
**Главное правило (один способ на API):** динамика Hexa и данные задаются **только** через **`handlers`**: на форме — **`onFormInit`** / **`onFormSubmit`**; на узлах с Hexa-конфигом — **`handlers.useConfig`**. Резолв: **прямая функция** **`(slice) => …`**, ленивый **`() => import('…')`** (для **`useConfig`** — затем **`useConfigs[control.id]`** в модуле), либо **строка пути** к **`.ts`** + **`loadTsModule`** при открытом каталоге формы. После **`parseFormTs`** относительные импорты схемы переписываются в ленивые **`import()`**; если **`import()`** из **blob** не сработал, тот же путь подгружается с диска (**`lifecycle-resolve`** + FSA). Подробно: **[feat-schema-handlers.md](./features/feat-schema-handlers.md)**. Нет форменного **`configHook`** и общего реестра без явного **`useConfig`** на узле.
...
2. **Динамика пропсов:** для выбранных контролов внешний вид и поведение задаются **`handlers.useConfig`** (именованный хук из модуля, общий ленивый импорт + **`useConfigs[id]`**, или путь к **`.ts`**). Контейнеры (**grid**, **table**) сохраняют в DSL структуру (матрица, `children`), оформление Hexa — в модуле хуков, без дублирования в DSL.
...
- **Модуль хуков** (часто **`{formId}.config-hook.ts`**): **как** узлы выглядят в Hexa и как реагирует форма — экспорты **`useConfigs`**, именованные **`use…`**, **`onFormInit`** / **`onFormSubmit`** в **`*.data.ts`** и т.д. (см. **[feat-schema-handlers](./features/feat-schema-handlers.md)**). Слой **продукта разработки**: типы, логика, API. Отдельный файл от схемы упрощает версионирование и ревью.
