...
| `configHook` | `string` | Путь к модулю configHook, **только `.ts`** ([tooling.md](./tooling.md)). |
...
После введения **п.2** [roadmap.md](./roadmap.md) ожидается, что идентификаторы полей в `state` (например константа вроде `BUTTON_TEXT_FIELD_ID` в примере) и структура `elements` будут сверяемы с типом корневого объекта формы — без расхождения между `.js` формой и типами в `form-dsl.ts`.
