...
**Готово.** Динамическая конфигурация — один TS-модуль на форму (каталог **`dsl/{id}/`**: **`{id}.schema.ts`**, **`{id}.config-hook.ts`**), диспатч по **`control.id`**, без хука на контролах в DSL. **Конвенция:** UI-пропсы Hexa не дублируются в схеме (например **`input`** — в DSL id + привязки; плейсхолдер и подпись — в хуке). План: [plan-form-config-hook-341.md](./plans/plan-form-config-hook-341.md), фича: [feat-form-config-hook-unified.md](./features/feat-form-config-hook-unified.md). **Актуальный контракт исполнения** (**`handlers`**, **`useConfig`**, **`parseFormTs`**, FSA): [feat-schema-handlers.md](./features/feat-schema-handlers.md).

### 3.4. Один `configHook` на форму — дальше

#### ✅ Итерация 16.04:

    3.4.1. ✅ починить типизацию elementId в хуке
    3.4.2. ✅ добавить data.ts файл для формы, описывающий получение, отправку и моки данных, onInit и onSubmit перенести туда
    3.4.3. ✅ confighook должен экспортировать из себя функции, эти функции должны подключаться в настройках формы и компонентов
    <a id="normalization-roadmap-p344"></a>
    3.4.4. ✅ вынести биндинг к данным в UI, передавать это свойство модели в конфигхук компонента — план: [plan-form-model-binding.md](./plans/plan-form-model-binding.md)
    <a id="normalization-roadmap-p345"></a>
    3.4.5. ✅ вынести настройки disabled, visible в UI, сделать биндинг к данным — план: [plan-form-visible-disabled.md](./plans/plan-form-visible-disabled.md)
    3.4.6. ✅ вынести валидацию в ui, сделать биндинг к функции (**`handlers.onFormValidate`**, debounce по state, гейт submit, ошибки под **`control.id`**)
    3.4.7. ✅ конфигурация всех handlers в tool  браузере и открытие файла по ссылке из браузера
    3.4.8. ✅ проверить удобство биндинга к данным
        - ✅ именование modelBindPath: 
        - ✅ динамическое переопределение disabled, visible, valid из хука
        - ✅ удобство настройки модели в UI tool
        - ✅ отображение в wysiwyg
    

#### Итерация 07.05: Стабилизация

-2. Собрать форму: список установленных продуктов на хосте
-1. Без хука компонент должен рендериться по-умолчанию
0. Проработать связь данных с хендлерами
1. Исправить баги сохранения (сейчас осхраняется json вместо ts)
2. Исправить неудобство выставления условий: нельзя задать if true
3. Найти и исправить баги работы, написать e2e тесты основных сценариев
4. Есть ли сквозная типизация schema? Тайпинг нужно улучшить, чтобы тип каждой настройки задавался явно и однозначно
...
- Привести общие настройки в соответствие с механиками Билдера
